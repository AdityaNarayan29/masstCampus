...
export async function middleware(request: NextRequest) {
  ...
  try {
    ...
    if (data.success && data.data) {
      // Tenant found - add tenant ID to request headers
      ...
      // Tenant not found - redirect to error page or default tenant
      ...
      // Option 2: Redirect to default tenant (uncomment if needed)
      // return NextResponse.redirect(new URL('http://localhost:3000', request.url));
    }
  } catch (error) {
    console.error('Tenant resolution error:', error);
    // Continue without tenant resolution on error
    return NextResponse.next();
  }
}
